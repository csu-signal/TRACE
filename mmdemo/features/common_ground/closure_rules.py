...
class CommonGround:
    ...
    def update(self, move, content):
        ...
        for prop in props:
            ...
            prop_match = re.match(
                r"(red|blue|green|yellow|purple)\s*(=|<|>|!=)\s*(.*)", prop
            )
            if prop_match:
                block = prop_match[1]
                ...
                if len(rhs_blocks) == 1 and (
                    len(self.poss[block]) < len(self.poss[rhs_blocks[0]])
                    or (
                        len(self.poss[block]) == len(self.poss[rhs_blocks[0]])
                        and len(
                            self.evidence_for[block].union(self.evidence_against[block])
                        )
                        > len(
                            self.evidence_for[rhs_blocks[0]].union(
                ...
                if "STATEMENT" in move:
                    if relation == "=":
                        ...
                        # elif block = sum(blocks)
                            ...
                            if len(rhs_blocks) == 1:
                                for poss_weight in self.evidence_for[rhs_blocks[0]]:
                                    self.evidence_for[block].add(poss_weight)
                                for not_weight in self.evidence_against[rhs_blocks[0]]:
                                    self.evidence_against[block].add(not_weight)
                            else:
                                # find possible values of sum(blocks)
                                poss_blocks = list(
                                    self.poss[rhs_block] for rhs_block in rhs_blocks
                                )
                                poss_weights = set(
                                    sum(weights) for weights in product(*poss_blocks)
                                )
                                ...
                                ev_weights = set(
                                    sum(weights) for weights in product(*ev_blocks)
                                )
                                # add evidenced weights to
                                # self.evidence_for[block]
                                for poss_weight in self.poss[block].intersection(
                                    ev_weights
                                ):
                                    self.evidence_for[block].add(poss_weight)
                    elif relation == "<":
                        ...
                        if rhs_weight:
                            ...
                            poss_weights = filter(
                                lambda x: x < rhs_weight, self.poss[block]
                            )
                            # add impossible weights to
                            # self.evidence_against[block]
                            for not_weight in {10, 20, 30, 40, 50}.difference(
                        ...
                        # elif block < sum(blocks)
                            ...
                            if len(poss_weights) > 1:
                                poss_weights_2 = filter(
                                    lambda x: x < max(poss_weights), self.poss[block]
                                )
                            else:
                        ...
                        # elif block > sum(blocks)
                            ...
                            if len(poss_weights) > 1:
                                poss_weights_2 = filter(
                                    lambda x: x > min(poss_weights), self.poss[block]
                                )
                            else:
                                poss_weights_2 = poss_weights
                            # add impossible weights to
                ...
                # ACCEPTs remove impossible weights
                    ...
                    if relation == "=":
                        ...
                        if rhs_weight:
                            # find possible weights
                            poss_weights = filter(
                                lambda x: x < int(rhs_weight), self.poss[block]
                            )
                            # remove impossible weights
                            for not_weight in {10, 20, 30, 40, 50}.difference(
                                poss_weights
                    ...
                    if relation == "=":
                        if rhs_weight in self.evidence_for[block] or rhs_blocks:
                            for weight in {10, 20, 30, 40, 50}.difference({rhs_weight}):
                                self.poss[block].add(weight)
                    elif relation == "<":
                        poss_weights = filter(
                            lambda x: x >= rhs_weight, {10, 20, 30, 40, 50}
                        )
