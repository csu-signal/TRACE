...
def load_frame_azure(
    frame, framergb, bodies, rotation, translation, cameraMatrix, dist, shift
):
    faces_li = []
    heads = []
    images = []
    ...
    for _, body in enumerate(bodies):
        box, nosex, nosey = headBoundingBox(
            body, rotation, translation, cameraMatrix, dist
        )
