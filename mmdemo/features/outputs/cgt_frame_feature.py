...
@final
class CGTFrame(BaseFeature[ColorImageInterface]):
    """
    Return the output frame used for common ground tracking
    ...
    def __init__(
        ...
        super().__init__(color, gesture, sel_objects, common_ground, calibration)
    ...
    def get_output(
        ...
        if self.has_cgt_data or common.is_new():
            ...
            CGTFrame.renderBanks(output_frame, 130, 260, "FBank", common.fbank)
            CGTFrame.renderBanks(output_frame, 130, 130, "EBank", common.ebank)
        else:
            CGTFrame.renderBanks(output_frame, 130, 260, "FBank", set())
            CGTFrame.renderBanks(output_frame, 130, 130, "EBank", set())
    ...
    def projectVectorLines(cone: Cone, frame, calibration, includeY, includeZ, gaze):
        ...
        """
        baseUpY, baseDownY, baseUpZ, baseDownZ = CGTFrame.conePointsBase(cone)
        vertexUpY, vertexDownY, vertexUpZ, vertexDownZ = CGTFrame.conePointsVertex(
            cone
        )
    ...
    def renderBanks(frame, xSpace, yCord, bankLabel, bankValues):
        ...
        for i in range(1, blocks):
            ...
            labels = CGTFrame.getPropValues(bankValues, color.name)
