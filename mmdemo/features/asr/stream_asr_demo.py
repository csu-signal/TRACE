...
def recognize(wavfile):
    ...
    speech = (
        data.astype(np.float16) / 32767.0
...
def asr():
    ...
    while keepWorking:
        try:
            data = q.get(
                block=True, timeout=1 if not first else None
            )  # wait infinitely long on first call
            first = False
        except queue.Empty:
            break
...
print("start")
with sd.InputStream(
    samplerate=16000, channels=1, blocksize=640, callback=rec_sound, dtype=np.int16
):
    sd.sleep(20000)
print("stop")
keepWorking = False
