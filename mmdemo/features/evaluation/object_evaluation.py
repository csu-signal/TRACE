...
from demo.featureModules import ObjectFeature
from demo.featureModules.utils import Block, GamrTarget
    ...
    def __init__(self, input_dir, log_dir=None):
        ...
        with open(input_dir / self.LOG_FILE, "r") as f:
            ...
            for row in reader:
                ...
                p20 = float(data["p20"])
                p21 = float(data["p21"])
                self.blocks_by_frame[frame_index].append(
                    Block(class_gamr, [p10, p11], [p20, p21])
                )
