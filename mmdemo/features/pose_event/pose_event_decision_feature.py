    ...
    def get_output(
        ...
        if self.live:
            for body in self.positive_cooling_time:
                self.positive_cooling_time[body] -= current_time - self.last_frame_time if self.positive_cooling_time[body] > 0 else 0
            for body in self.negative_cooling_time:
                self.negative_cooling_time[body] -= current_time - self.last_frame_time if self.negative_cooling_time[body] > 0 else 0
        else:
            for body in self.positive_cooling_time:
                self.positive_cooling_time[body] -= 1 / self.frame_rate if self.positive_cooling_time[body] > 0 else 0
            for body in self.negative_cooling_time:
                self.negative_cooling_time[body] -= 1 / self.frame_rate if self.negative_cooling_time[body] > 0 else 0
    ...
    def event_decision(self):
        ...
        for body in self.queue:
            ...
            while begin < len(self.queue[body]) - 1:
                if self.queue[body][-1][1] - self.queue[body][begin][1] >= self.leanout_count_time and self.queue[body][-1][1] - self.queue[body][begin + 1][1] < self.leanout_count_time:
                    break
                begin += 1
            
            if begin < len(self.queue[body]) - 1:
                leanout = all(i[0][0] != "leaning in" for i in self.queue[body][begin + 1:])
                ...
                if self.negative_cooling_time[body] <= 0.00001:
                    if leanout:
                        negative_event += 1
                        file.write(str(self.time) + "," + str(body) + " negative posture event\n")
                ...
                if self.positive_cooling_time[body] <= 0.00001:
                    if not leanout:
                        positive_event += 1
                        file.write(str(self.time) + "," + str(body) + " positive posture event\n")
