...
class Menu(QWidget):
    ...
    def save(self):
        ...
        self.write_file()
        QMessageBox.about(
            self, "Parameters Saved", "Your parameters are successfully saved."
        )
    ...
    def initUI(self):
        ...
        label5 = QLabel(
            "How long to play the video from its beginning (unit: second)", self
        )
        self.textboox5 = QLineEdit(str(self.config["playback_time"]), self)

        label6 = QLabel(
            "Pick out a participant who is speaking (from left to right, they are P1|P2|P3)",
            self,
        )
        self.textboox6 = QLineEdit(self.config["speaker"], self)

        label7 = QLabel(
            "Position of the divider that separates left participant and middle participant",
            self,
        )
        self.textboox7 = QLineEdit(str(self.config["left_position"]), self)

        label8 = QLabel(
            "Position of the divider that separates middle participant and right participant",
            self,
        )
        self.textboox8 = QLineEdit(str(self.config["middle_position"]), self)

        label9 = QLabel(
            "How long gaze history stored for each participant (unit: second)", self
        )
        self.textboox9 = QLineEdit(str(self.config["gaze_window"]), self)

        label10 = QLabel(
            "Recorded as event when a participant did/didn't look at the speaker for ? seconds",
            self,
        )
        self.textboox10 = QLineEdit(str(self.config["individual_gaze_event"]), self)

        label11 = QLabel(
            "Recorded as event when two participants did/didn't look at the speaker for ? seconds",
            self,
        )
        self.textboox11 = QLineEdit(str(self.config["group_gaze_event"]), self)

        label12 = QLabel(
            "Buffer used when one participant switches from no gaze to gaze (unit: second)",
            self,
        )
        self.textboox12 = QLineEdit(str(self.config["gaze_beginning_buffer"]), self)

        label13 = QLabel(
            "Buffer used when one participant switches from gaze to no gaze (unit: second)",
            self,
        )
        self.textboox13 = QLineEdit(str(self.config["gaze_lookaway_buffer"]), self)

        label14 = QLabel(
            "How long one received feature can override the following None information frame\nUsed to smooth features and reduce no information frame(unit: second)",
            self,
        )
        self.textboox14 = QLineEdit(str(self.config["smooth_frame"]), self)

        label15 = QLabel(
            "How long posture history stored for each participant (unit: second)", self
        )
        self.textboox15 = QLineEdit(str(self.config["pose_window"]), self)

        label16 = QLabel(
            "Report positive posture event ? seconds after last report", self
        )
        self.textboox16 = QLineEdit(str(self.config["pose_positive_event"]), self)

        label17 = QLabel(
            "Report negative posture event ? seconds after last report", self
        )
        self.textboox17 = QLineEdit(str(self.config["pose_negative_event"]), self)

        label18 = QLabel(
            "Recorded as a negative posture event if Leaning out for more than ? seconds",
            self,
        )
        self.textboox18 = QLineEdit(str(self.config["leanout_time"]), self)

        label19 = QLabel(
            "How frequently to update behavioral engagement level (unit: second)", self
        )
        ...
        self.textboox22 = QLineEdit(
            str(self.config["posture_positive_count_time"]), self
        )

        label23 = QLabel("Posture negative event count down time (unit: second)", self)
        self.textboox23 = QLineEdit(
            str(self.config["posture_negative_count_time"]), self
        )

        self.checkbox1 = QCheckBox(
            "Draw gaze cones on frame (after setting, please save)", self
        )
        self.checkbox1.setChecked(self.config["draw_gaze_cone"])

        self.checkbox2 = QCheckBox(
            "If selected, run live analysis; If not selected, run play back video (after setting, please save)",
            self,
        )
