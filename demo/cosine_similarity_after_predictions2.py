###This version is used when the model is trained, and the output stored, but the whole program isn't finished. Basically,
###the model takes a long time finish, and this was written while debugging the cosine simimlarity portion.
import numpy as np
...
if __name__ == "__main__":
    ...
    predictions = []
    targets = []
    ###read in the predictions from depth_est_bignet_ablation2/depth_est_bignet_ablation2_test_Group10
    ...
    predictions = [element for row in predictions for element in row] ###flatten to a 1D list
    print("len(predictions)",len(predictions))
    print("len(predictions[0])",len(predictions[0]))
    print("len(predictions[1])",len(predictions[1]))
    print("len(predictions[2])",len(predictions[2]))
    ...
    with open('cosine_similarity_test_Group10_nose_eyes_ears.csv','w') as f:
        writer = csv.writer(f)
