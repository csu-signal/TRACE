...
class MoveFeature:
    ...
    def __init__(
        self,
        log_dir=None,
        model=str(Path(__file__).parent / "production_move_classifier.pt"),
    ):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.model = torch.load(str(model), pickle_module=custom_pickle).to(self.device)
        self.model.eval()
