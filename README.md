...
- After installing, add the file path, `C:Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.8` (often the dafault installation path) to your system environment variables under `Path`. Additionally, check that the paths for `CUDA_PATH` and `CUDA_PATH_V12_6`match the newly added file path.
