import csv
from featureModules import MoveFeature
...
class MoveFeatureEval(MoveFeature):
    ...
    def __init__(self, input_dir, log_dir=None):
        ...
        with open(input_dir / self.LOG_FILE, "r") as f:
            ...
            for row in reader:
                ...
                move = []
                if include_statement: move.append("STATEMENT")
                if include_accept: move.append("ACCEPT")
                if include_doubt: move.append("DOUBT")
    ...
    def processFrame(self, frame, new_utterances: list[int], utterance_lookup: list[UtteranceInfo] | dict[int, UtteranceInfo], frameIndex, includeText):
        ...
        if includeText:
            cv2.putText(frame, "Move evaluation", (50, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
